...
```{r}
#log-standardise variables

data_grouped <- data_grouped %>%
  mutate(log_n = log(n + 1),
         log_non_resident_ratio = log(non_resident_ratio + 1),
         log_resident_ratio = log(resident_ratio + 1))

#linear model
model <- lm(log_n ~ log_non_resident_ratio + log_resident_ratio, data = data_grouped)
summary(model)
